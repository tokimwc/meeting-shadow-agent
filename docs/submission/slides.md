...
**Title:** Following the request is not checking it

An engineer on an English delivery call may understand the request and still reply before four
...
The engineer reads it and decides. The agent never speaks or messages anyone in the meeting. The client's pushback on the next turn
...
- On the slide, the second box says the memo is self-declared: the engineer's own statement of what they
  may agree to, not checked against the authority their company gave them. (The code also drops an open
  item whose only evidence is the memo.)
...
and one did not move. Not on the slide, but in the long description with its conditions: repeating one configuration, scoring the decision
...
**Title:** The engineer feels it. The delivery lead is the likely buyer.
...
- **Likely buyer:** the delivery lead at a software services firm, who absorbs the unpaid rework when a
...
|---|---|---|
| "Let me check and get back to you" | Simple deferral, no tool needed | The engineer still decides, alone and mid-call, when to use it |
| The memo, kept beside the call | The boundary in view | Matching it to the request, in English, mid-call |
| Copilot in Teams · Ask Gemini in Meet | Private Q&A through the engineer's own prompts | Does not know the boundary unless told; Copilot only in meetings your own organisation hosts |
| **This agent** | A reply shaped by the declared boundary, after every turn | Can produce wrong replies; not yet tried on a live call |
...
- **Wrong agreements.** In the audio run, 4 of 20 final cards agree to what the memo withholds or the
  speaker left undefined, and the model's own commitment flag has been wrong once. *Next: score every card, not only
  the last.*
