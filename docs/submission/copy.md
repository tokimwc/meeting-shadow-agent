...
with the memo, and a conversation where everything is already settled. 58 of 62 turns returned
a suggestion; over those, median 2.02 s from end of speech to suggestion, p90 2.55 s, in-process.
...
run, 4 of 20 final cards agreed to something the memo withholds or never defined, and scenarios built by the person
...
screen halfway through a request sometimes agrees to it even when the final decision is right. An open
item citing only the memo is dropped, but one citing the memo plus any spoken id is kept. And when a
client changes the question after a refusal, the card can keep deferring on the old one.
...
over-commitment made in a sentence on a Tuesday call turns into unbilled weekends, a renegotiation,
